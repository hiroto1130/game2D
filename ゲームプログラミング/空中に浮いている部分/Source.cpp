...
HRESULT InitD3d(HWND hInst, const TCHAR* filePath)
{
	...
	if (FAILED(D3DXCreateTextureFromFile(pDevice, "brick_100x100.png", &pTexture2)))
	{
		return E_FAIL;
	}
	return S_OK;
...
void keymove()
{
	//GravityCount = GravityCount - 1.0f;
	//v[0].y -= GravityCount;
	//v[1].y -= GravityCount;
	//v[2].y -= GravityCount;
	//v[3].y -= GravityCount;
	...
	if (JumpFlag == true)
	{
		...
		if ((v[3].y > 200 && v[3].y < 350) && (150 < v[0].x && v[1].x < 350))
		{
			if (GravityCount + SpeedCount < 0)
			{
				GravityCount = 0;
				SpeedCount = 0;
				JumpFlag = false;
...
void Judgment()
{
	...
	// ブロックの当たり判定
	  // 下の当たり判定 
	/*if ((  v[0].y < 300 && v[1].y > 200) && (150 < v[0].x && v[1].x < 350)) 
	{
		v[0].y = 300.0f;
		v[1].y = 300.0f;
		v[2].y = 350.0f;
